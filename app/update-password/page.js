...
export default function UpdatePasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Supabase handles the session from the email click. We just need to update the user.
        ...
        <div ref={formSectionRef} className="max-w-xl w-full bg-gradient-to-b from-stone-950/95 to-stone-950/100 border border-stone-850/80 backdrop-blur-md rounded-3xl p-8 md:p-10 shadow-2xl flex flex-col justify-center">
          ...
          {successMsg ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-stone-300 mb-2">New Password</label>
                <input 
                  type="password" 
                  required
                  value={password}
              ...
              <div className="text-xs text-stone-400 font-medium leading-relaxed pt-1">
                Make sure it's at least 6 characters long and something you haven't used recently.
              </div>
